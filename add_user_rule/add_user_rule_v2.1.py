...
## 系统判断
if platform != 'win32':
    print('%s ERROR 000: only support Desktop Windows OS.' %
    ...
    if not os.path.isfile(abs_path):
        ...
        with open(abs_path, 'w', encoding='UTF-8') as fd_pac:
            ...
            print('%s INFO: %s: new file created.' %
                  (time.strftime('[%Y-%m-%d_%H:%M:%S]'), my_pac_fn))
...
## 检查自定义 pac 文件存在否
chkMyPacExist(v2n_path, my_pac_file)

## 用指定的文本编辑器打开自定义 pac 文件
my_pac_path = v2n_path + os.sep + my_pac_file

print('%s INFO: opening %s with %s editor ...' % (
      time.strftime('[%Y-%m-%d_%H:%M:%S]'), my_pac_file, editor))
print('%s INFO: ADD YOUR OWN RULE LINE BY LINE IN %s FILE.' % (
      time.strftime('[%Y-%m-%d_%H:%M:%S]'), my_pac_file))
print('%s INFO: WHEN YOU ADD RULE COMPLETE OR ADD NOTHING, SAVE IT THEN CLOSE %s EDITOR.' % (
      time.strftime('[%Y-%m-%d_%H:%M:%S]'), editor))
input('%s INFO: Press Enter to continue: ' % time.strftime('[%Y-%m-%d_%H:%M:%S]'))
print('%s INFO: waiting until %s exit ...' % (
      time.strftime('[%Y-%m-%d_%H:%M:%S]'), editor))
...
input('%s INFO: Press Enter to Exit. ' % time.strftime('[%Y-%m-%d_%H:%M:%S]'))
